...
function extractFacts(payload, lead) {
  ...
  return {
    ...
    // ── Niveau de vie (MOD 1) — réutilise les données DÉJÀ présentes dans le payload,
    //    AUCUN nouveau calcul financier :
    //      • ACTUEL   = revenu net mensuel courant (monthlySalaryNet) ;
    //      • PROJETÉ  = pensionNetteMensuelle (déjà extrait) ;
    //      • ÉCART    = manqueMensuel (déjà extrait) ;
    //      • SOUHAITÉ = objectif retraite, reconstruit EXACTEMENT depuis deux valeurs existantes :
    //                   pension + manque  (inverse de « manque = max(0, objectif − pension) »).
    niveauVieActuel: num(s.monthlySalaryNet),
    niveauVieSouhaite: (num(s.pensionNetteMensuelle) != null && num(pd.manqueMensuel) != null)
...
function buildSystemPrompt() {
  return [
    ...
    '  },',
    '  "conseiller": {',
    '    "opportuniteCommerciale": "2-3 phrases INTERNES et ACTIONNABLES (PAS un résumé) : pourquoi appeler ce lead MAINTENANT et ce qui en fait une vraie opportunité PER (levier fiscal lié à la TMI, plafond PER, écart à financer, capacité d\'épargne, horizon).",',
    '    "anglePrincipal": "1-2 phrases : l\'argument d\'ouverture le plus percutant pour CE profil, formulé prêt à être dit au téléphone.",',
    '    "anglesSecondaires": ["élément de persuasion 1", "élément de persuasion 2"],',
    '    "objectionProbable": "1 phrase : l\'objection la plus probable de ce profil (ex. épargne bloquée jusqu\'à la retraite, « je verrai plus tard », capacité d\'épargne, doute sur le rendement…).",',
    '    "reponseObjection": "1-2 phrases : la réponse recommandée à cette objection, factuelle, rassurante et prête à l\'emploi, sans sur-promettre.",',
...
function buildUserMessage(f, temperature) {
  ...
  lines.push('');
  lines.push('NIVEAU DE VIE (cœur de l\'analyse prospect — à exploiter EN PRIORITÉ pour le bloc "impactNiveauVie") :');
  if (f.niveauVieActuel != null) lines.push('- Niveau de vie net ACTUEL : ' + eur(f.niveauVieActuel) + ' / mois');
  if (f.niveauVieSouhaite != null) lines.push('- Niveau de vie SOUHAITÉ à la retraite : ' + eur(f.niveauVieSouhaite) + ' / mois' + (f.lifestylePct != null ? ' (objectif fixé à ' + f.lifestylePct + ' % du revenu de fin de carrière)' : ''));
  if (f.pensionNetteMensuelle != null) lines.push('- Niveau de vie PROJETÉ par la simulation (pension) : ' + eur(f.pensionNetteMensuelle) + ' / mois');
  if (f.manqueMensuel != null) lines.push('- ÉCART RÉEL à financer chaque mois : ' + eur(f.manqueMensuel) + ' / mois');
  lines.push('');
